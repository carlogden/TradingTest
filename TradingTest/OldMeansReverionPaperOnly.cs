    ...
    class MeanReversionPaperOnly
    {
        ...
        public void Run()
        {
            ...
            while (timeUntilClose.TotalMinutes > 15)
            {
                ...
                Console.WriteLine($"Current price: {currentPrice} average price {avg}");

                if (diff <= 0)
                {
                    Console.WriteLine($"Diff {diff} is below zero, exit if applicable");
                ...
                else
                {
                    ...
                    Console.WriteLine($"Diff {diff} is greater than zero, amount to add {amountToAdd}");
